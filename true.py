```
# ==================== COMPLETE true.py WITH ALL FEATURES ====================
# Features:
# 1. Number lookup via Truecaller bot
# 2. TG lookup via username-usrid-to-num API
# 3. 5-minute caching for both
# 4. Multi-account round-robin
# 5. API key management
# 6. Admin panel
# 7. Auto access refresh via Nick_Bypass_Bot
...
- 5-minute caching for fast repeated lookups
"""
...
class SearchCache:
    ...
    def set(self, key, data):
        ...
        with self._lock:
            ...
            print(f"[CACHE] STORED: {key} (expires in {self._ttl}s)")
    ...
    def stats(self):
        ...
        with self._lock:
            ...
            active = sum(1 for v in self._cache.values() if now < v[1])
            return {
                "total_entries": total,
...
def init_db():
    ...
    """)
    # Add daily_limit columns if not exists
    try:
        c.execute("ALTER TABLE api_keys ADD COLUMN daily_limit INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE api_keys ADD COLUMN daily_uses INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE api_keys ADD COLUMN last_reset TEXT")
    except: pass
...
def fetch_user_data_from_api(tg_id_or_username):
    """
    Fetch user data from username-usrid-to-num API
    Returns: name, username, telegram_id, phone, country, dc_location, registered_date, registered_ago, era
    """
    import requests as _req
    
    query = tg_id_or_username.strip().lstrip('@')
    
    if query.isdigit():
        endpoint = f"/userid={query}"
    else:
        endpoint = f"/username={query}"
    
    try:
        url = f"{USERID_API}{endpoint}?key={USERID_API_KEY}"
        print(f"[TG] API Call: {url}")
        
        r = _req.get(url, timeout=15)
        print(f"[TG] API status: {r.status_code}")
        ...
        if r.status_code == 200:
            ...
            if data.get("status") == True:
                ...
                if not full_name:
                    full_name = data.get("username", "Unknown")
                
                # Format registered ago
                reg_ago = data.get("registered_ago", "")
                if reg_ago and "ago" not in reg_ago:
                    reg_ago = f"{reg_ago} ago"
                
                result = {
                    "success": True,
                    "name": full_name,
                    "username": f"@{data.get('username', '')}" if data.get('username') else None,
                    "telegram_id": data.get("tg_id"),
                    "phone": phone,
                    "country": data.get("country", "Unknown"),
                    "dc_location": data.get("dc_location", "Unknown"),
                    "registered_date": data.get("reg_date", ""),
                    "registered_ago": reg_ago,
                    "era": data.get("era", ""),
                    "made_by": "@Felix_Bhai",
                    "source": "api"
                }
                print(f"[TG] API found: {full_name} | {phone}")
                return result
            else:
                print(f"[TG] API returned status false: {data.get('error', 'Unknown error')}")
                return None
        else:
...
def num_lookup():
    ...
    # ── CHECK CACHE FIRST ──
    cache_key = f"num_{num_c}"
    cached = search_cache.get(cache_key)
    if cached:
        print(f"[CACHE] Number lookup served from cache: {num_c}")
        cached["cached"] = True
...
def tg_lookup():
    ...
    tg = request.args.get('tg', '').strip()
    if not tg:
        return jsonify({"success": False, "error": "Missing tg parameter"}), 400

    # ── CHECK CACHE FIRST ──
    cache_key = f"tg_{tg.strip().lstrip('@')}"
    cached = search_cache.get(cache_key)
    if cached:
        print(f"[CACHE] TG lookup served from cache: {tg}")
        cached["cached"] = True
        return _Resp(_json.dumps(cached, ensure_ascii=False), mimetype='application/json')

    # ── TRY API FIRST ──
    api_data = fetch_user_data_from_api(tg)
    
    if api_data and api_data.get("success"):
        # ── STORE IN CACHE ──
        search_cache.set(cache_key, api_data)
        api_data["cached"] = False
        return _Resp(_json.dumps(api_data, ensure_ascii=False), mimetype='application/json')
    
    # ── FALLBACK: TELEGRAM RESOLUTION ──
    print(f"[TG] API failed, falling back to Telegram: {tg}")
    ...
    if is_username:
        ...
        if not tg_info:
            tg_info = {"name": f"User {tg}", "username": None, "telegram_id": tg, "public_phone": None}

    tg_id = tg_info["telegram_id"]
    print(f"[TG] Resolved: {tg_info}")

    # Get phone from public profile
    phone = tg_info.get("public_phone")
    if phone:
        phone = phone
    else:
        phone = "Not found"
...
def home():
    ...
    return _R(
        '{"status":true,"name":"FELIX API","version":"3.0","developer":"@felix_bhai","maintenance":true,"maintenance_message":"system"}',
...
<div class="wrap">
  ...
  <div class="stats-row">
    ...
    <div class="stat"><div class="ico">📊</div><div class="val" id="st-cache">—</div><div class="lbl">Cache Entries</div></div>
  ...
  <div class="card">
    <div class="card-hd">
      <h2><span class="dot"></span>Cache Management</h2>
    </div>
    <div class="btn-row">
      <button class="danger" onclick="clearCache()">🗑️ Clear Cache</button>
      <button class="ghost" onclick="loadCacheStats()">📊 Refresh Stats</button>
    </div>
    <div id="cacheInfo" style="margin-top:12px;font-size:.82rem;color:var(--tx2)"></div>
  </div>
...
async function loadStats(){
  ...
  document.getElementById('st-cache').textContent = d.cache_entries ?? '—';
  document.getElementById('st-uptime').textContent = '✅';
}

async function loadCacheStats(){
  const d = await api('/admin/cache');
  if(d.success){
    document.getElementById('cacheInfo').innerHTML = 
      `📦 Total: ${d.total_entries} | Active: ${d.active_entries} | Expired: ${d.expired_entries} | TTL: ${d.ttl_seconds}s`;
    document.getElementById('st-cache').textContent = d.total_entries ?? '—';
  }
}

async function clearCache(){
  if(!confirm('Clear all cached data?')) return;
  const d = await api('/admin/cache/clear','POST');
  if(d.success){ toast('Cache cleared: ' + d.cleared_entries + ' entries', 'ok'); loadCacheStats(); loadStats(); }
...
loadStats(); loadKeys(); loadAccounts(); loadCacheStats();
...
async def main():
    ...
    print("📡 http://localhost:5000/api?key=felix&tg=USERNAME")
    print("📡 http://localhost:5000/api/tg?key=felix&tg=USERID")
