...
- Parallel TG lookup (Shadow API + Public Profile — fastest wins)
"""
...
STRING_SESSION = os.environ.get("STRING_SESSION", "1BVtsOLMBuxyZuG7x8HzrK_PPr7JJieTM5DmqoJXdClXsNANzvHnPmKM4ScXTdYrF8tkta6Hlc0Vjv1MYLZR9pn9iABwqG49WwyZ1yQ33PQsYakNkh_kEM8yVfKtZf5ElDUITcn0H3PkVLfwKdjru3lJUSPVj1mOO0QLGglvCOY-TAOB_ZxZ2238Ix-_gO0kgsNacG-oJYKOKEHfnbSZJb1COOoVH2T8M2D6qMAb86lTUvej_7pWjbid7M54qHqAaoAl3MUJ_guZDCtUosmYn3D1fRx94WaUkvboThKjFK55kKEjlpWC2xhdWBAoa0HmsMeM_qJQ1ksiGJxht5gsWFOj4day6iGk=")
...
# Shadow OSINT API (FASTEST)
SHADOW_API = "https://shadowosintapi.co08.art/api/developer/Cyb3rB4nn3r/fast"
SHADOW_KEY = "c797993aa04e03df3b6d597c001be4f3"
...
# ==================== TG LOOKUP (PARALLEL - FASTEST WINS) ====================

def fetch_phone_shadow(tg_id):
    """Shadow OSINT API — parallel execution ke liye"""
    try:
        import requests as _req
        url = f"{SHADOW_API}?key={SHADOW_KEY}&userid={tg_id}"
        print(f"[SHADOW] Requesting: {url}")
        r = _req.get(url, timeout=5)
        print(f"[SHADOW] Status: {r.status_code}")
        if r.status_code == 200:
            d = r.json()
            if d.get("phone"):
                print(f"[SHADOW] ✅ Found: {d.get('phone')} ({d.get('country')})")
                return {
...
async def fetch_public_phone(tg_info):
    """Public profile phone (instant)"""
    pub = tg_info.get("public_phone")
    if pub and pub not in (None, "", "null"):
        return {
            "country": "Unknown",
            "country_code": "",
            "phone_number": pub,
...
async def fetch_parallel(tg_id, tg_info):
    """Run both sources in parallel — return first successful result"""
    
    # Create tasks for both sources
    tasks = [
        asyncio.create_task(asyncio.to_thread(fetch_phone_shadow, tg_id)),
        asyncio.create_task(fetch_public_phone(tg_info))
    ]
    ...
    while pending:
        ...
        for task in done:
            result = task.result()
            if result and result.get("phone_number") not in (None, "", "Not found", "null"):
                elapsed = int((time.time() - start_time) * 1000)
                print(f"[PARALLEL] ✅ Got from {result.get('source')} in {elapsed}ms")
        ...
        # Agar sab tasks complete ho gaye aur koi result nahi mila
        if not pending:
            break
    
    # Default: not found
    return {"country": "Unknown", "country_code": "", "phone_number": "Not found", "source": "None", "response_ms": "0"}
...
def tg_lookup():
    ...
    # Step 1: Resolve to TG info (via Telethon)
    ...
    # Step 2: Fetch phone — PARALLEL (both at same time)
    location = run_async(fetch_parallel(tg_id, tg_info))
    
    # Step 3: info_key based on input type
    info_key = "username_info" if is_username else "userid_info"

    # Step 4: Fix phone number — remove country code prefix if present
    phone_num = location["phone_number"]
    country_code = location["country_code"]
    country = location["country"]

    # Agar public phone hai aur country_code missing hai — detect kar lein
    ...
    # Step 5: Build response — FASTEST RESPONSE
    result = {
        info_key: {
            "name":        tg_info["name"],
        ...
        "made_by": "@Felix_Bhai",
        "response_ms": location.get("response_ms", "0")
    }
